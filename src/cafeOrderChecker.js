function isFirstComeFirstServed(takeOutOrders, dineInOrders, servedOrders) {
  ...
  let tIndex = 0;
  let dIndex = 0;
  const tMaxIndex = takeOutOrders.length - 1;
  const dMaxIndex = dineInOrders.length - 1;

  for (const order of servedOrders) {
    if (tIndex <= tMaxIndex && order === takeOutOrders[tIndex]) {
      tIndex++;
    } else if (dIndex <= dMaxIndex && order === dineInOrders[dIndex]) {
      dIndex++;
    } else {
      return false;
    }
  ...
  if (tIndex !== takeOutOrders.length || dIndex !== dineInOrders.length) {
    return false;
  }
